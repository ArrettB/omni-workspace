...
-- Contacts
CREATE TABLE #GPtoServiceTraxContacts(
	[contact_id] [numeric](19, 5) NOT NULL,
...
INSERT INTO #GPtoServiceTraxContacts(contact_id,NewGPExtDealerID)
SELECT contacts.contact_id, a.NewPrimaryKey1
FROM contacts
INNER JOIN (
	...
	INNER JOIN organizations org ON
		CASE b.INTERID
			...
			WHEN 'AMMAD' THEN 'WISC'
   			...
   			WHEN 'CIMN' THEN 'CIMN'
			ELSE '' END = org.code
) a ON contacts.organization_id = a.organization_id and contacts.ext_dealer_id = a.OldPrimaryKey1
GROUP BY contacts.contact_id, a.NewPrimaryKey1
ORDER BY contacts.contact_id

--update the table 
	update contacts 
	SET contacts.ext_dealer_id = map.NewGPExtDealerID
	FROM contacts
	INNER JOIN #GPtoServiceTraxContacts map ON contacts.contact_id = map.contact_id


--CLEANUP
DROP TABLE #GPtoServiceTraxContacts
