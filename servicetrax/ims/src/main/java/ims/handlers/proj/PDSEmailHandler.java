...
//	private final static String SECURITY = "security_contact_id";
    ...
    public enum EmailTemplate {
        ...
        QR_SR_SENT(
                "To: ${contact_name} on ${todays_date}" + NEWLINE
                        ...
                        + "Est End Date: ${est_end_date}" + NEWLINE
                        + "Customer PO: ${customer_po_no}"  + NEWLINE
                        + "Description: ${description}" + NEWLINE),
    ...
    public static final String SELECT
            ...
            + "       ISNULL(?, convert(varchar(10), est_end_date, 101)) est_end_date,"
            + "       ISNULL(?,'None') customer_po_no,"
    ...
    public boolean handleEnvironment(InvocationContext ic) throws Exception {
        ...
        if (est_start_date != null && est_start_date.length() == 0) {
            est_start_date = null;
        }
        if (est_end_date != null && est_end_date.length() == 0) {
            est_end_date = null;
        }
        ...
        String customer_po_no = ic.getParameter("customer_po_no");
        ...
        Object[] params = new Object[]{vendor_contact_name, sch_start_date, est_start_date, est_end_date, customer_po_no, refusal_reason, recordId, record_type};
        QueryResults rs = conn.select(query, params);

        if (rs.next()) {// returned record of request or quote
            ...
            if (StringUtil.hasAValue(record_type_code)) {
                if (!record_type_code.equalsIgnoreCase("workorder")) {// is a service, quote request or a quote.
                    ...
                    if (is_new_version) {
                        ...
                        if (oldQuoteStyle == null) {
                            oldQuoteStyle = Boolean.FALSE;
                        }
                ...
                else {// must be a workorder

                    if (send_survey) {// need to send A&M survey to customer
                        ...
                        if (record_status_type_code.equalsIgnoreCase("unapproved")) {
                        ...
                        else if (record_status_type_code.equalsIgnoreCase("needs_quote")) {// notify a_m of new workorder quote request
                            emails.put(A_M, EmailTemplate.A_M_WORKORDER_QUOTE_REQUEST);
                        }
                        else if (record_status_type_code.equalsIgnoreCase("approved")) {// notify customer of approval if not a vendor update

                            if (StringUtil.hasAValue(request_vendor_id)) {// vendor updated his schedule

                                emails.put(CUSTOMER, EmailTemplate.CUSTOMER_NEW_SCHEDULE);
                                emails.put(ALT_CUSTOMER, EmailTemplate.CUSTOMER_NEW_SCHEDULE);
                            }
                            else if (is_new_version) {// notify vendors/scheduler/customer of new version
                            ...
                            else {// normal customer/vendor approval notification
            ...
            while (email_enum.hasMoreElements()) {
                ...
                if (StringUtil.hasAValue(contact_id = rs.getString(contact_column))) {// contact column has a value so we should email them
...
//	}
    ...
    private StringBuffer emailSchedulerTest(InvocationContext ic, ConnectionWrapper conn) throws Exception {
        ...
        if (rs.next()) {// they are not equal so notify scheduler of only the fields that have changed
        ...
        else {// they are not equal notify scheduler
            result = null;
        }
