...
public class ServiceStatus extends AbstractDatabaseObject
{
	...
	public static ServiceStatus fetch(String statusID, InvocationContext ic)
	{
		 return fetch(Integer.parseInt(statusID), ic, null);
	}

	public static ServiceStatus fetch(String statusID, InvocationContext ic, String resourceName)
	{
		 return fetch(Integer.parseInt(statusID), ic, resourceName);
	}

	public static ServiceStatus fetch(int statusID, InvocationContext ic)
	{
		 return fetch(statusID, ic, null);
	}

	public static ServiceStatus fetch(int statusID, InvocationContext ic, String resourceName)
	{
		Diagnostics.trace("ServiceStatus.fetch()");
		...
		try
		{
			...
			query.append(", code ");
			query.append(", name ");
			query.append("FROM service_statuses ");
			...
			{
				Diagnostics.error("Error in ServiceStatus.fetch(), Could not find ServiceStatus; Select was:" + query);
			}
		}
		catch (Exception e)
		{
			ErrorHandler.handleException(ic, e, "Exception in ServiceStatus.fetch()");
		}
		finally
		{
			if (conn != null)
	...
	{
		Diagnostics.trace("ServiceStatus.internalUpdate()");
		...
		try
		{
			...
			update.append("UPDATE service_statuses ");
		...
		{
			ErrorHandler.handleException(ic, e, "Exception in ServiceStatus.internalUpdate()");
		}
		finally
		{
			if (conn != null)
	...
	{
		Diagnostics.trace("ServiceStatus.internalInsert()");
		...
		try
		{
			...
			if (skipPrimary)
			{
				insert.append("INSERT INTO service_statuses (");
				insert.append("code");
				insert.append(", name");
		...
		{
			ErrorHandler.handleException(ic, e, "Exception in ServiceStatus.internalInsert()");
		}
		finally
		{
			if (conn != null)
	...
	{
		Diagnostics.trace("ServiceStatus.internalDelete()");
		...
		try
		{
			...
			delete.append("DELETE FROM service_statuses ");
		...
		{
			ErrorHandler.handleException(ic, e, "Exception in ServiceStatus.internalDelete()");
		}
		finally
		{
			if (conn != null)
