...
// Source File Name:   SmartFieldComponent.java
...
public class SmartFieldComponent extends TemplateComponent
{
	...
	public SmartFieldComponent() throws Exception
	{
	...
	public String includeInternal(InvocationContext ic) throws Exception
	{
		...
		boolean canView = "true".equals(((String) ic.getTransientDatum("View")));
		boolean canSave = "true".equals(((String) ic.getTransientDatum("Save")));
		...
		if (displaySize > 0)
			length = " maxlength=\"" + displaySize + "\"";
		boolean isMandatory = false;
		String mandatoryString = getString(ic, "mandatory");
		if (mandatoryString != null)
			isMandatory = getBoolean(ic, "mandatory");
		else if (meta != null)
			isMandatory = meta.isColumnMandatory(name);
		...
		{
			if (resourceName != null && connTemp != null)
				connTemp.release();
		}
		if (isCalculated)
		{
	...
	public static String formatDate(String date)
	{
		...
		if (!StringUtil.hasAValue(result))
			result = convertToDateString("dd-mmm-yyyy", "MM/dd/yyyy", date);
		if (!StringUtil.hasAValue(result))
			result = convertToDateString("MM/dd/yyyy", null, date);
		return result;
	}
	...
	static String formatDateTime(String date)
	{
		...
		if (!StringUtil.hasAValue(result))
			result = convertToDateString("MM/dd/yyyy h:mm a", null, date);
		return result;
	}
	...
	static String formatTime(String date)
	{
		...
		if (result == null)
			result = convertToDateString("h:mm a", null, date);
		else if (result == null)
			result = convertToDateString("h:mm a", null, date);
		return result;
	}
	...
	private String createDropList(InvocationContext ic, ConnectionWrapper conn, String name, String typename, boolean canSave, boolean isReadOnly, boolean isMandatory, boolean isError,
			String fieldResource) throws Exception
	{
		StringBuffer result = new StringBuffer();
		...
		PreparedStatement pstmt = IMSUtil.queryToPreparedStatement(conn, new StringBuffer(query));
		ResultSet psRS = pstmt.executeQuery();

		for (rs = new QueryResults(conn, pstmt, psRS); rs.next();)
		{
	...
	private String createChooser(InvocationContext ic, ConnectionWrapper conn, String name, String typename, boolean canSave, boolean isReadOnly, boolean isMandatory, boolean isError,
			String fieldResource) throws Exception
	{
		StringBuffer result = new StringBuffer();
		...
		if (query == null && primaryKeyId != null)
			query = "SELECT " + key + " id, " + display + " display  FROM " + table + " where " + key + "=" + StringUtil.toPStmtString(primaryKeyId);
		String valueFromKey = null;
		// new prepared statement code
		PreparedStatement pstmt = IMSUtil.queryToPreparedStatement(conn, new StringBuffer(query));
		ResultSet psRS = pstmt.executeQuery();
		QueryResults rs;
		for (rs = new QueryResults(conn, pstmt, psRS); rs.next() && StringUtil.hasAValue(primaryKeyId);)
	...
	private String createDateTime(InvocationContext ic, ConnectionWrapper conn, String name, String typename, boolean canSave, boolean isReadOnly, boolean isMandatory, boolean isError)
			throws Exception
	{
		StringBuffer result = new StringBuffer();
	...
	private String createTextArea(InvocationContext ic, ConnectionWrapper conn, String name, String typename, String length, boolean canSave, boolean isReadOnly, boolean isMandatory, boolean isError)
			throws Exception
	{
		StringBuffer result = new StringBuffer();
		String value = getValue(ic, typename);
	...
	private String createBoolean(InvocationContext ic, ConnectionWrapper conn, String name, String typename, boolean canSave, boolean isReadOnly, boolean isMandatory, boolean isError)
			throws Exception
	{
		StringBuffer result = new StringBuffer();
		String value = getValue(ic, typename);
	...
	private String createMoney(InvocationContext ic, ConnectionWrapper conn, String name, String typename, String length, boolean canSave, boolean isReadOnly, boolean isMandatory, boolean isError)
			throws Exception
	{
		StringBuffer result = new StringBuffer();
	...
	private String createPercent(InvocationContext ic, ConnectionWrapper conn, String name, String typename, String length, boolean canSave, boolean isReadOnly, boolean isMandatory, boolean isError)
			throws Exception
	{
		StringBuffer result = new StringBuffer();
	...
	private String createText(InvocationContext ic, ConnectionWrapper conn, String name, String typename, String length, boolean canSave, boolean isReadOnly, boolean isMandatory, boolean isError)
			throws Exception
	{
		StringBuffer result = new StringBuffer();
		String value = getValue(ic, typename);
	...
	private String getCSS(InvocationContext ic, String name, String value, boolean canSave, boolean isReadOnly, boolean isMandatory, boolean isError) throws Exception
	{
		String type = getString(ic, "type");
	...
	private String getEvents(InvocationContext ic) throws Exception
	{
		...
		boolean useCSS = parent.getBoolean(ic, "useCSS");
		if (useCSS)
			return " onFocus=\"fixColors()\" onBlur=\"fixColors()\"";
		else
			return "";
	}
	...
	public SmartFormComponent getParentSmartForm() throws Exception
	{
		for (TemplateComponent t = getParent(); t != null; t = t.getParent())
			if (t instanceof SmartFormComponent)
				return (SmartFormComponent) t;
