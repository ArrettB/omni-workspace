...
<html>
<head>
<title><fmt:message key="servicetrax.tc.time.title"/></title>
...
<!--//
	...
	var myRowCreater = function(options)
	{
		...
		{
		    row.className="warn";
			addEvent(row, "click", function(event) { rowClicked(row, id, event); });
		}
	...
	function fillTable(data)
	{
		...
		var getId = function(timeRecord) { return timeRecord.id };
		var getJobString = function(timeRecord) { return timeRecord.jobString };
		var getServiceString = function(timeRecord) { return timeRecord.serviceString };
		var getResourceString = function(timeRecord) { return timeRecord.resourceString };
		var getItemString = function(timeRecord) { return timeRecord.itemString };
		var getPaycodeString = function(timeRecord) { return timeRecord.paycodeString };
		var getQty = function(timeRecord) { return timeRecord.qty.toFixed(1) };
		var getDate = function(timeRecord) { return formatDate(timeRecord.date) };
...
<h2 id="title"><fmt:message key="servicetrax.tc.time.title"/></h2>
...
<table class="formTable">
	<tbody>
		<tr>
			...
			<td></td>
			<td align="right" rowspan="2" colspan="2"><div id="indicator" ><img src="<c:url value="/images/indicator_medium.gif"/>" alt="indicator" /></div></td>
		</tr>
		<tr>
		...
		<tr>
			...
			<td class="control"><select id="reqs"><option/></select></td>
		</tr>
		<tr>
			<td class="label"><fmt:message key="servicetrax.tc.time.form.item"/></td>
			<td class="control"><select id="items"><option/></select></td>
		</tr>
		<tr>
			<td class="label"><fmt:message key="servicetrax.tc.time.form.paycode"/></td>
			<td class="control"><select id="paycodes"><option/></select></td>
		</tr>
		<tr>
			<td class="label"><fmt:message key="servicetrax.tc.time.form.qty"/></td>
			<td class="control"><input type="text" id="qty" /></td>
		</tr>
		<tr>
			<td colspan="4" align="left">
			<input type="button" name="edit_button" id="edit_button" value="" onclick="addRecord()" />
			<input type="button" name="cancel_button" id="cancel_button" value="<fmt:message key="servicetrax.tc.time.form.cancel"/>" onclick="cancelEdit()" />
			</td>
		</tr>
	</tbody>
</table>
...
<input type="button" name="Clear" value="<fmt:message key="servicetrax.tc.time.table.clear"/>" onclick="clearRecords()" />
