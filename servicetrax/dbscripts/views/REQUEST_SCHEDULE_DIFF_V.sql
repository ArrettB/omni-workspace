...
SELECT     requests_a.REQUEST_ID, requests_b.REQUEST_ID AS REQUEST_ID_B, ISNULL(requests_a.SCHEDULE_TYPE_ID, - 1) AS SCHEDULE_TYPE_ID, 
                      ...
                      dbo.CUSTOMERS.VIEW_SCHEDULE_FLAG, LOOKUPS_1.CODE AS request_type_code
FROM         dbo.LOOKUPS schedule_types RIGHT OUTER JOIN
                      dbo.CUSTOMERS INNER JOIN
                      dbo.PROJECTS ON dbo.CUSTOMERS.CUSTOMER_ID = dbo.PROJECTS.CUSTOMER_ID RIGHT OUTER JOIN
                      dbo.REQUESTS requests_a INNER JOIN
                      dbo.REQUESTS requests_b ON requests_a.PROJECT_ID = requests_b.PROJECT_ID AND 
                      requests_a.REQUEST_NO = requests_b.REQUEST_NO INNER JOIN
                      dbo.LOOKUPS LOOKUPS_1 ON requests_b.REQUEST_TYPE_ID = LOOKUPS_1.LOOKUP_ID ON 
                      dbo.PROJECTS.PROJECT_ID = requests_b.PROJECT_ID LEFT OUTER JOIN
                      dbo.LOOKUPS schedule_types_b ON requests_b.SCHEDULE_TYPE_ID = schedule_types_b.LOOKUP_ID ON 
                      schedule_types.LOOKUP_ID = requests_a.SCHEDULE_TYPE_ID LEFT OUTER JOIN
                      dbo.CONTACTS contacts_b ON requests_b.CUSTOMER_CONTACT_ID = contacts_b.CONTACT_ID LEFT OUTER JOIN
                      dbo.JOB_LOCATIONS job_locations_b ON requests_b.JOB_LOCATION_ID = job_locations_b.JOB_LOCATION_ID
WHERE     (dbo.CUSTOMERS.VIEW_SCHEDULE_FLAG = 'Y') AND (LOOKUPS_1.CODE = 'workorder') OR
                      (LOOKUPS_1.CODE = 'service_request')
