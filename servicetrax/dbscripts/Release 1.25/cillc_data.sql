...
SELECT o.organization_id, 
       (SELECT custnmbr FROM cillc.dbo.rm00101 WHERE RTRIM(shrtname) LIKE 'CI, ILL%' AND userdef1='DEALER'),
       (SELECT custname FROM cillc.dbo.rm00101 WHERE RTRIM(shrtname) LIKE 'CI, ILL%' AND userdef1='DEALER'),
       custnmbr,shrtname,
       'Y',
       getdate(),
       1,
...
UPDATE customers 
 ...
 WHERE ext_dealer_id = (SELECT custnmbr FROM cillc.dbo.rm00101 WHERE RTRIM(shrtname) LIKE 'CI, ILL%' AND userdef1='DEALER') 
   AND ISNULL(ext_customer_id,'') NOT IN (SELECT custnmbr FROM cillc.dbo.rm00101 WHERE shrtname NOT LIKE 'CI, ILL%' AND userdef1='DEALER')
   AND organization_id=(SELECT organization_id FROM organizations WHERE db_prefix='cillc.dbo.')
GO
                         ...
                         WHERE l1.code='end_user'
                           AND c1.end_user_parent_id IN (SELECT c.customer_id 
                                                          ...
                                                          WHERE lt.code = 'customer_type'
                                                            AND l.code='dealer')
                           AND c1.organization_id=(SELECT organization_id FROM organizations WHERE db_prefix='cillc.dbo.'))
  AND l.code='active'
  AND jl.active_flag='Y'
GO

EXECUTE sp_cillc_dealer_contact
GO
   
EXECUTE sp_cillc_new_end_user
GO
...
INSERT INTO job_location_contacts (job_location_id,
  ...
  SELECT jl.job_location_id, c.contact_id, 1, getdate()
   ...
   WHERE l.code='active'
     ...
     AND eu.ext_dealer_id = (SELECT custnmbr FROM cillc.dbo.rm00101 WHERE RTRIM(shrtname) LIKE 'CI, ILL%' AND userdef1='DEALER')
     AND eu.end_user_parent_id IN (SELECT c.customer_id
                                    ...
                                    WHERE l.code IN ('commercial','corporate','government')
                                      ...
                                      AND c.organization_id=(SELECT organization_id FROM organizations WHERE db_prefix='cillc.dbo.')
