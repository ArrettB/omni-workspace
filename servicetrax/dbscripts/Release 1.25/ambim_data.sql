...
SELECT o.organization_id, 
       (SELECT custnmbr FROM ambim.dbo.rm00101 WHERE RTRIM(shrtname)='A&M Business In' AND userdef1='DEALER'),
       'A&M Business In',
       custnmbr,shrtname,
       'Y',
       getdate(),
...
UPDATE customers 
 ...
 WHERE ext_dealer_id = (SELECT custnmbr FROM ambim.dbo.rm00101 WHERE RTRIM(shrtname)='A&M Business In' AND userdef1='DEALER') 
   AND ISNULL(ext_customer_id,'') NOT IN (SELECT custnmbr FROM ambim.dbo.rm00101 WHERE shrtname NOT LIKE 'A&M%' AND userdef1='DEALER')
   AND organization_id=(SELECT organization_id FROM organizations WHERE db_prefix='ambim.dbo.')
GO
                         ...
                         WHERE l1.code='end_user'
                           AND c1.end_user_parent_id IN (SELECT c.customer_id 
                                                          ...
                                                          WHERE lt.code = 'customer_type'
                                                            AND l.code='dealer')
                           AND c1.organization_id=(SELECT organization_id FROM organizations WHERE db_prefix='ambim.dbo.'))
  AND l.code='active'
  AND jl.active_flag='Y'
GO
   
EXECUTE  sp_ambim_dealer_contact
GO
   
EXECUTE sp_ambim_new_end_user
GO
...
SELECT   jl.job_location_id, c.contact_id, 1, getdate()
   ...
   WHERE l.code='active'
     ...
     AND eu.ext_dealer_id = (SELECT custnmbr FROM ambim.dbo.rm00101 WHERE RTRIM(shrtname)='A&M Business In' AND userdef1='DEALER')
     AND eu.end_user_parent_id IN (SELECT c.customer_id
                                    ...
                                    WHERE l.code IN ('commercial','corporate','government')
                                      ...
                                      AND c.organization_id=(SELECT organization_id FROM organizations WHERE db_prefix='ambim.dbo.')
